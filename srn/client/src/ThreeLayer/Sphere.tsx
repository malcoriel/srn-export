...
const flareFragmentShader = require('./flareFragmentShader.txt').default;
const flareVertexShader = require('./flareVertexShader.txt').default;

export const ShaderTest = (props: any) => {
  useFrame(() => {
    shaderParams.u_frame.value += 1.0;
  });

  return (
    <group {...props}>
      <mesh>
        <planeBufferGeometry args={[20, 1]} />
      </mesh>
    </group>
  );
};
...
> = (props) => {
  ...
  return (
    <mesh {...props} ref={mesh} rotation={[Math.PI / 2, Math.PI, 0]}>
      ...
      <shaderMaterial attach="material" args={[SimplexNoise]} />
    </mesh>
  );
};
